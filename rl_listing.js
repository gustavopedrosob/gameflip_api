...
async function main() {
    ...
    let photo_url = process.argv[2];
    let tags = [      // Must use the correct tag for search/filtering to function properly
          `id: ${process.argv[6]}`,
          `type: ${process.argv[7]}`,
          `quantity: ${process.argv[8]}x`
        ];
    // Example with color
    // tags: [ "id: chakram", "type: Wheel", "color: Black" ]
    if (process.argv[12] !== "") {
        tags.push(`color: ${process.argv[12]}`);
    }
    if (process.argv[13] !== "") {
        tags.push(`certification: ${process.argv[13]}`);
    }
    ...
    let query = {
        ...
        price: process.argv[5], // price in cents
        tags: tags,
        // Example with color
        // tags: [ "id: chakram", "type: Wheel", "color: Black" ]
        ...
        upc: GfApi.UPC.RL_STEAM,
        platform: GfApi.PLATFORM.STEAM,
        shipping_within_days: process.argv[9],
        expire_in_days: process.argv[10],
    ...
    gfapi.upload_photo(listing.id, photo_url, 0).then(() => {
      ...
      return gfapi.listing_status(listing.id, process.argv[11]);
    }).catch(err => {
      console.log(err);
    });
}
